...
data Param = Pos Int | Imm Int | Rel Int
             deriving (Eq,Ord,Show)

run :: [Int] -> [Int] -> [Int]
run prog input = run' (prepare prog) input 0 0
  where
    prepare :: [Int] -> Map Int Int
    prepare = Map.fromList . zip [0..]

    run' :: Map Int Int -> [Int] -> Int -> Int -> [Int]
    run' prog inps p base = case opcode of 
      1  -> run' (set prog (addr z) ((val x) + (val y))) inps (p+4) base
      2  -> run' (set prog (addr z) ((val x) * (val y))) inps (p+4) base
      3  -> run' (set prog (addr x) (head inps)) (tail inps) (p+2) base
      4  -> (val x) : run' prog inps (p+2) base
      5  -> if (val x) /= 0 then run' prog inps (val y) base else run' prog inps (p+3) base
      6  -> if (val x) == 0 then run' prog inps (val y) base else run' prog inps (p+3) base
      7  -> if (val x) < (val y)  then run' (set prog (addr z) 1) inps (p+4) base else run' (set prog (addr z) 0) inps (p+4) base
      8  -> if (val x) == (val y) then run' (set prog (addr z) 1) inps (p+4) base else run' (set prog (addr z) 0) inps (p+4) base
      9  -> run' prog inps (p+2) (base + (val x))
      99 -> []
      where
        (opcode,xm,ym,zm) = 
          let intcode = reg p
              code  = intcode `mod` 100
              modes = intcode `div` 100
              xm = modes `mod` 10
              ym = (modes `div` 10) `mod` 10
              zm = (modes `div` 10) `div` 10
          in (code,xm,ym,zm)
        ...
        set prog i v = Map.insert i v prog

        reg i = Map.findWithDefault 0 i prog
...
main = do putStr "Part 01: "
          print $ head $ run prog [1]
          putStr "Part 02: "
          print $ head $ run prog [2]
